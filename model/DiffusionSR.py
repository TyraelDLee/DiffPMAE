...
class Diff_Point_MAE(nn.Module):
    def __init__(self, config):
        ...
        self.mask_token_sr = nn.Conv1d((self.num_channel * self.num_output) // self.num_group, self.trans_dim,  1)
        ...
        self.increase_dim_sr = nn.Sequential(
    ...
    def forward(self, x_0, t, x_vis, mask, center, ori, debug=False):
        """
        x_0: lr full (vis, msk)
        t: time step
        x_vis: lr vis latent
        ...
        mask_token = self.mask_token_sr(x_t.reshape(B, self.num_group, -1).transpose(1, 2)).transpose(1, 2).to(x_vis.device)
        x_full = mask_token[:, :N, :]
        x_full = torch.cat([x_vis, x_full], dim=1)
        # x_full = torch.cat([x_vis, mask_token], dim=1)  # x_vis, x_vis, msk
        x_rec = self.decoder(x_full, pos_full, self.num_group, ts)
        x_rec = self.increase_dim_sr(x_rec.transpose(1, 2)).transpose(1, 2).reshape(B, -1, 3)
        if debug:
            return x_t, noise, x_rec
        else:
            return self.loss_func(x_rec, ori)
    ...
    def sampling_t(self, x, t, mask, center, x_vis):
        ...
        pos_full = torch.cat([pos_emd_vis, pos_emd_msk], dim=1)
        _, N, _ = pos_emd_msk.shape
        mask_token = self.mask_token_sr(x.reshape(B, self.num_group, -1).transpose(1, 2)).transpose(1, 2).to(x_vis.device)
        x_full = mask_token[:, :N, :]
        x_full = torch.cat([x_vis, x_full], dim=1)
        x_rec = self.decoder(x_full, pos_full, self.num_group, ts)
        x_rec = self.increase_dim_sr(x_rec.transpose(1, 2)).transpose(1, 2).reshape(B, -1, 3)
    ...
    def sampling(self, x_vis, mask, center, ret=False, noise_patch=None):
        B, M, C = x_vis.shape
        if noise_patch is None:
            noise_patch = torch.randn((B, self.num_output, 3)).to(x_vis.device)
        traj = []
