...
class Group(nn.Module):  # FPS + KNN
    def __init__(self, num_group, group_size):
        ...
        self.knnhr = KNN(k=self.group_size*4, transpose_mode=True)

    def forward(self, xyz, hr=None):
        '''
            input: B N 3
            ---------------------------
            output: B G M 3
        ...
        if hr is not None:
            _, hr_points, _ = hr.shape
            _, hidx = self.knnhr(hr, center)
            hidx_base = torch.arange(0, batch_size, device=xyz.device).view(-1, 1, 1) * num_points
            hidx = hidx + hidx_base
            hidx = hidx.view(-1)
            h_n = hr.view(batch_size * hr_points, -1)[hidx,:]
            h_n = h_n.view(batch_size, self.num_group, self.group_size*4, 3).contiguous()
            h_n = h_n - center.unsqueeze(2)
            return neighborhood, center, h_n
        else:
...
class Encoder_Module(nn.Module):
    ...
    def encode(self, pt, hr=None, masked=False):
        B, _, N = pt.shape
        if masked:
            if hr is None:
                neighborhood, center = self.group_divider(pt)
            else:
                neighborhood, center, h_n = self.group_divider(pt)
            x_vis, mask, x_masked = self.AE_encoder(neighborhood, center)
            return x_vis, mask, center
        else:
            if hr is not None:
                neighborhood, center, h_n = self.group_divider(pt, hr)
                x_vis, mask, x_masked = self.AE_encoder(neighborhood, center)
                vis_pc, msk_pc = self.neighborhood(neighborhood, center, mask, x_vis)
                vis_hr, msk_hr = self.neighborhood(h_n, center, mask, x_vis)
                return x_vis, x_masked, mask, center, vis_pc, msk_pc, vis_hr, msk_hr
            else:
                neighborhood, center = self.group_divider(pt)
                x_vis, mask, x_masked = self.AE_encoder(neighborhood, center)
                vis_pc, msk_pc = self.neighborhood(neighborhood, center, mask, x_vis)
